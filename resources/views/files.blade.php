                        ...
                        <div class="card-body image-thumb">
                            <div class="mb-4 text-center p-3 rounded iq-thumb">
                                ...
                                <a href="#" data-title="{{$model->filename}}" data-load-file="file" data-load-target="#resolte-contaniner" data-url="/uploads/{{$model->filename}}" data-toggle="modal" data-target="#exampleModal">
                                   ...
                                   <img src="../assets/images/layouts/page-7/pdf.png" class="img-fluid" alt="image1"></a>   
                                   @endif   
                                   @if ($model->filetype == 'docx')
                                   <img src="../assets/images/layouts/page-7/doc.png" class="img-fluid" alt="image1"></a>   
                                   @endif
                                   @if ($model->filetype == 'ppt' || $model->filetype == 'ppt')
                                   <img src="../assets/images/layouts/page-7/ppt.png" class="img-fluid" alt="image1"></a>   
                                   @endif   
                            </div>
                           {{-- BURADA ISIM DEGISECEK    --}}
                           <span ondblclick="openModal(this,{{$model->id}})">{{$model->filename}}</span>

                           <!-- Input Modalı -->
                           <div id="myModal_{{$model->id}}" class="modal closethis_{{$model->id}}">
                             <div class="my-modal-content">
                           
                             <form action="/files/edit/{{$model->id}}" method="GET">
                                <input type="text" id="textInput" value="{{$model->filename}}" class="input-{{$model->id}}" name="filename">
                                <button type="submit">Değiştir</button>
                            </form>
                            <button class="closeBtn" onclick="test({{$model->id}})">Kapat</button>
                             </div>
                           </div>
                                      ...
                                      <tr>
                                        <td>
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div data-load-file="file" data-load-target="#resolte-contaniner" data-url="../upload/{{$model->filename}}" data-toggle="modal" data-target="#exampleModal" data-title="alexa5.pdf" style="cursor: pointer;">{{$model->filename}}</div>
                                            </div>
                                        </td>
                                        <td>Me</td>
...
@section('js')
<script>
function openModal(element,modelId) {
 
  // Modalı görünür hale getir
  var modal = document.getElementById("myModal_"+modelId);
  modal.style.display = "block";
}
var closeBtn = document.querySelector('.closeBtn')
  function test (modelId) {
  var modal = document.querySelector('.closethis_'+modelId);
